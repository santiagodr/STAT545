...
```{r}
typeof(typeof(15.2)) #the first output is a character so the second object is also a character
```
...
```{r}
nrow(iris)
dim(iris) #gives you entire dimension of table
```
...
```
```{r}
filter(gapminder, (country == "Canada") | (country == "Algeria" & year %in% 1960:1969)) #other way
```
...
gapminder %>% 
  arrange(year, desc(lifeExp)) %>% 
  select(year, lifeExp, everything()) # to rearrange order of columns but keep everything else
```
