...
```{r}
library(RColorBrewer) # load first

gapminder %>% 
  ggplot(aes(x = lifeExp, fill = continent)) +
  geom_histogram() +
  facet_wrap(~continent) +
  scale_fill_manual(values = brewer.pal(n=5, "Pastel2"))
```
Second option
```{r}
...
```{r}
colour_layer <- scale_colour_manual("", #empty "" to avoid title
                                    labels = c("Other coutries", "Rwanda"),
                                    values = c("black", "red"))
gapminder%>% 
...
In this case, we can use `==` to give another condition that returns FALSE or TRUE, thats a new variable in the colour parameter, but we want to modify the legend as well, so we habe to create that first part
